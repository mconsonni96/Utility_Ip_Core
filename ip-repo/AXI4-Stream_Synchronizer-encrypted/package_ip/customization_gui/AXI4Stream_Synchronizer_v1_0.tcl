...
proc init_gui { IPINST } {
  ...
  set MODE_SEL [ipgui::add_param $IPINST -name "MODE_SEL" -parent ${Page_0} -widget comboBox]
  set_property tooltip {Choice between the CDC_Synchronizer and the FIFO_Synchronizer} ${MODE_SEL}
  #Adding Group
  set FIFO_Settings [ipgui::add_group $IPINST -name "FIFO Settings" -parent ${Page_0}]
  set RELATED_CLOCKS [ipgui::add_param $IPINST -name "RELATED_CLOCKS" -parent ${FIFO_Settings}]
  set_property tooltip {It specifies if the two clocks are related or not} ${RELATED_CLOCKS}
  set FIFO_DEPTH [ipgui::add_param $IPINST -name "FIFO_DEPTH" -parent ${FIFO_Settings}]
  set_property tooltip {Write depth of the FIFO} ${FIFO_DEPTH}
  set FIFO_MEMORY_TYPE [ipgui::add_param $IPINST -name "FIFO_MEMORY_TYPE" -parent ${FIFO_Settings} -widget comboBox]
  ...
  set CDC_SYNC_STAGES [ipgui::add_param $IPINST -name "CDC_SYNC_STAGES" -parent ${FIFO_Settings}]
  set_property tooltip {Number of synchronization stages in the CDC path} ${CDC_SYNC_STAGES}

  #Adding Group
  set CDC_Settings [ipgui::add_group $IPINST -name "CDC Settings" -parent ${Page_0}]
  set SRC_SYNC_FF [ipgui::add_param $IPINST -name "SRC_SYNC_FF" -parent ${CDC_Settings}]
  set_property tooltip {Number of synchronization stages in the CDC path of the source(slave)} ${SRC_SYNC_FF}
  set DEST_SYNC_FF [ipgui::add_param $IPINST -name "DEST_SYNC_FF" -parent ${CDC_Settings}]
...
proc update_PARAM_VALUE.CDC_SYNC_STAGES { PARAM_VALUE.CDC_SYNC_STAGES PARAM_VALUE.MODE_SEL } {
	...
	if { [gen_USERPARAMETER_CDC_SYNC_STAGES_ENABLEMENT $values(MODE_SEL)] } {
