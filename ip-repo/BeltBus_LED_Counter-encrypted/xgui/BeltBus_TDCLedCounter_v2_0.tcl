...
proc init_gui { IPINST } {
  ...
  ipgui::add_param $IPINST -name "RECOVERY_SPEED" -parent ${Page_0}
  ipgui::add_param $IPINST -name "CALIBRATION_SPEED" -parent ${Page_0}
  #Adding Group
...
proc update_PARAM_VALUE.CALIBRATION_SPEED { PARAM_VALUE.CALIBRATION_SPEED PARAM_VALUE.CONSTANT_LED_SPEED } {
	# Procedure called to update CALIBRATION_SPEED when any of the dependent parameters in the arguments change
	
	set CALIBRATION_SPEED ${PARAM_VALUE.CALIBRATION_SPEED}
	set CONSTANT_LED_SPEED ${PARAM_VALUE.CONSTANT_LED_SPEED}
	set values(CONSTANT_LED_SPEED) [get_property value $CONSTANT_LED_SPEED]
	if { [gen_USERPARAMETER_CALIBRATION_SPEED_ENABLEMENT $values(CONSTANT_LED_SPEED)] } {
		set_property enabled true $CALIBRATION_SPEED
	} else {
		set_property enabled false $CALIBRATION_SPEED
	}
}
...
proc update_PARAM_VALUE.RECOVERY_SPEED { PARAM_VALUE.RECOVERY_SPEED PARAM_VALUE.CONSTANT_LED_SPEED } {
	# Procedure called to update RECOVERY_SPEED when any of the dependent parameters in the arguments change
	
	set RECOVERY_SPEED ${PARAM_VALUE.RECOVERY_SPEED}
	set CONSTANT_LED_SPEED ${PARAM_VALUE.CONSTANT_LED_SPEED}
	set values(CONSTANT_LED_SPEED) [get_property value $CONSTANT_LED_SPEED]
	if { [gen_USERPARAMETER_RECOVERY_SPEED_ENABLEMENT $values(CONSTANT_LED_SPEED)] } {
		set_property enabled true $RECOVERY_SPEED
	} else {
		set_property enabled false $RECOVERY_SPEED
	}
}
